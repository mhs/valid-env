...
describe ValidEnv::OptionalEnvVar do
  describe '#boolean?' do
    it 'returns true when it is a boolean env var' do
      env_var = ValidEnv::EnvVar.new(:FOO, :boolean)
      expect(env_var.boolean?).to be(true)
    end

    it 'returns false otherwise' do
      env_var = ValidEnv::EnvVar.new(:FOO)
  ...
  describe '#==' do
    it 'is equal to an env var of the same class and key (ignoring type)' do
      env_var_1 = ValidEnv::OptionalEnvVar.new(:foo, :boolean)
      env_var_2 = ValidEnv::OptionalEnvVar.new(:foo, :string)
    ...
    it 'is not equal to another env var with a different key' do
      env_var_1 = ValidEnv::OptionalEnvVar.new(:foo)
      env_var_2 = ValidEnv::OptionalEnvVar.new(:bar)
    ...
    it 'is not equal to another env var with same key, but different class' do
      env_var_1 = ValidEnv::OptionalEnvVar.new(:foo)
      env_var_2 = ValidEnv::RequiredEnvVar.new(:foo)
      expect(env_var_1).to_not eq(env_var_2)
      expect(env_var_2).to_not eq(env_var_1)
    end
  end
  ...
  describe '#value' do
    ...
    it 'returns the default value when there is no value in the env' do
      env_var = ValidEnv::EnvVar.new(:FOO, :boolean, default: true)
